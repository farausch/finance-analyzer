...
@app.post("/transactions/remove-label")
def remove_label_from_transaction(transaction_id: int, label_id: int, db: Session = Depends(get_db)):
    return label_service.remove_label_from_transaction(db, transaction_id, label_id)
